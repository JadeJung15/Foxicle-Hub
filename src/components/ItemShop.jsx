import { ShoppingBag, Zap, TrendingUp, Layout, CheckCircle2, Circle, Square, Image as ImageIcon } from 'lucide-react'
...
function ItemShop() {
  ...
  const getIcon = (id) => {
    switch (id) {
      ...
      case 'aura_blue': return <Circle size={32} color="#a366ff" strokeWidth={3} />
      case 'border_gold': return <Square size={32} color="#ff9500" strokeWidth={3} />
      case 'bg_pixel': return <ImageIcon size={32} color="#5856d6" />
  ...
  return (
    <div className="item-shop container">
      ...
      <div className="items-grid">
        {shopItems.map(item => {
          const isCustomType = ['aura', 'border', 'background'].includes(item.type)
          const isOwned = (item.type === 'unlock' || isCustomType) && ownedUpgrades.includes(item.id)
          const isCurrentBuff = item.type === 'buff' && ownedUpgrades.includes(item.id)
          const { equippedAura, equippedBorder, equippedBackground, applyCustom } = useGameStore()

          const isEquipped = (item.id === equippedAura) || (item.id === equippedBorder) || (item.id === equippedBackground)

          return (
            <div key={item.id} className={`shop-card glass ${isOwned ? 'owned' : ''} ${isEquipped ? 'equipped' : ''}`}>
              <div className="card-top">
                <div className="icon-circle">
                  {getIcon(item.id)}
                </div>
                {isOwned && <span className="badge owned-badge"><CheckCircle2 size={14} /> 보유 중</span>}
                {isEquipped && <span className="badge equipped-badge">장착 중</span>}
              ...
              <div className="card-footer">
                ...
                {isOwned ? (
                  isCustomType ? (
                    <button
                      className={`btn ${isEquipped ? 'btn-disabled' : 'btn-primary'}`}
                      onClick={() => applyCustom(item.type, item.id)}
                      disabled={isEquipped}
                    >
                      {isEquipped ? '장착 됨' : '장착하기'}
                    </button>
                  ) : (
                    <button className="btn btn-disabled" disabled>이미 보유</button>
                  )
                ) : (
                  <button className="btn btn-primary" onClick={() => buyItem(item.id)}>구매하기</button>
                )}
              </div>
            </div>
          )
      ...
      <style>{`
        ...
        .equipped-badge { background: #fee2e2; color: #ef4444; }
